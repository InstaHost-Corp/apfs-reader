...
struct apfsutil_options
{
  ...
  const char* path;
  const char* output;
  unsigned int password_stdin_volume;
  char password_stdin[MAX_APFS_PASSPHRASE_LENGTH + 2];
  bool subvolumes;
};

static const char* s_szOutput;
    ...
    )
{
  ...
  if ( NULL == Path || 0 == *Path || 0 == strcmp(Path, "/") )
    pWorkDir = fs->m_RootDir;
  else
  {
    CDir* Parent = GetParent( fs->m_RootDir, Path );
    if ( NULL == Parent || NULL == Path )
      return ERR_BADPARAMS;
    Status = Parent->OpenDir( api::StrUTF8, Path,
                              fs->m_Strings->strlen(api::StrUTF8, Path),
                              pWorkDir );
  }

  if ( UFSD_SUCCESS( Status ) )
  {
    Status = EnumerateFolderMachine( pWorkDir );
    if ( NULL != pWorkDir->m_Parent )
      pWorkDir->Destroy();
  }
  return Status;
...
{
  if ( NULL == s_szOutput || 0 == *s_szOutput )
  {
  ...
  FILE* Output = fopen( s_szOutput, "wb" );
  if ( NULL == Output )
  {
    CloseFile( File, Parent );
  ...
  size_t Bytes = 0;
  int Status = ERR_NOERROR;
  while ( UFSD_SUCCESS( Status = File->Read( Offset, Bytes, pReadBuf, BufSize ) )
          && Bytes != 0 )
  {
    if ( fwrite( pReadBuf, 1, Bytes, Output ) != Bytes )
    {
...
static const t_CmdHandler s_Cmd[] = {
  ...
  { "enum-machine"    , OnEnumFolderMachine },
  ...
  )
{
  ...
  for ( i = 1; i < argc; i++ )
  {
    ...
    else if ( 0 == strncmp( "--source=", a, 9 ) )
      opts->source = a + 9;
    else if ( 0 == strncmp( "--path=", a, 7 ) )
      opts->path = a + 7;
    else if ( 0 == strncmp( "--output=", a, 9 ) )
      opts->output = a + 9;
    else if ( 0 == strncmp( "--password-stdin=", a, 17 ) )
    {
      char* end = NULL;
      unsigned int v = strtoul( a + 17, &end, 0 );
      if ( v > MAX_APFS_VOLUMES || v == 0 || *end != 0 )
      {
        fprintf( stderr, "Invalid password volume number\n" );
        exit( -5 );
      }
      opts->password_stdin_volume = v;
    }
    ...
    )
{
  ...
  if ( 0 != opts.password_stdin_volume )
  {
#ifndef UFSD_WITH_OPENSSL
    fprintf( stderr, "--password-stdin doesn't work without OpenSSL\n" );
    return -1;
#else
    if ( NULL == fgets( opts.password_stdin, sizeof(opts.password_stdin), stdin ) )
    {
      fprintf( stderr, "Unable to read password\n" );
      return -5;
    }
    size_t passwordLength = strlen( opts.password_stdin );
    while ( passwordLength > 0
            && (opts.password_stdin[passwordLength - 1] == '\r'
                || opts.password_stdin[passwordLength - 1] == '\n') )
      opts.password_stdin[--passwordLength] = 0;
    opts.pass[opts.password_stdin_volume - 1] = opts.password_stdin;
#endif
  }

  s_szOutput = opts.output;
  if ( NULL != opts.source )
  {
    s_szDev = opts.source;
    s_szFile = opts.path;
  }
  else if ( SplitPath( argv[argc-1] ) == (const char*)0x1 )
  {
    OnUsage();
    return -1;
  }
