// <copyright file="assert.h" company="Paragon Software Group">
//
...
# define assert(e) do {} while((void)0,0)
...
#elif !defined NO_LOGGING && defined UFSD_TRACE_ASSERT && defined UFSD_USE_STATIC_TRACE && defined UFSD_TRACE

  extern "C" void ufsd_trace( const char* msg, ... ) __attribute__ ((format (printf, 1, 2)));
  ...
  #define assert(cond)                                      \
  do { if (!(cond)) {                                       \
      ufsd_trace( "***** assert " __FILE__ ", " __stringify(__LINE__) "\n" );    \
  }} while((void)0,0)
#else
  #define assert(cond)                                      \
  do { if (!(cond)) {                                       \
      ufsd_trace( "***** assert " __FILE__ ", " __stringify(__LINE__) ": %s\n", #cond );    \
   }} while((void)0,0)
#endif
...
#elif defined UFSD_DRIVER_LINUX
# error "Check you fs_conf file"
#else
# include <assert.h>
#endif
